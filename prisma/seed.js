...
async function main() {
  ...
  // Users: upsert is safe to re-run — won't overwrite existing data
  ...
  // Records: only seed if none exist — safe to re-run on every deploy
  const existingCount = await prisma.financialRecord.count();
  if (existingCount > 0) {
    console.log(`Records already seeded (${existingCount} found). Skipping.`);
  } else {
    const records = [
